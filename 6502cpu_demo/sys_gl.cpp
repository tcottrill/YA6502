/*
Example deprecated Opengl 2.0 Code. 
...
//----------------------------------TEXTURE Handling----------------------------------
//This is just a basic texture handler demo.
...
TEX *LoadPNG(std::string filename) 
{
	...
	// NPOT check
	if ((x & (x - 1)) != 0 || (y & (y - 1)) != 0) {
		wrlog("Note: texture %s is not power-of-2 dimensions\n", filename);
	}
	//Flip Texture
...
void ViewOrtho(int width, int height)
{
	...
	glOrtho(0, width, 980, 0, -1, 1);	  // Select Ortho 2D Mode (640x480)
